...
def fetch_unsplash_image(keyword: str, topic: str = "") -> dict:
    keywords_to_try = [keyword]
    if topic in TOPIC_FALLBACKS:
        keywords_to_try.append(TOPIC_FALLBACKS[topic])

    for kw in keywords_to_try:
        try:
            res = requests.get(
                "https://api.unsplash.com/search/photos",
                params={"query": kw, "per_page": 5, "orientation": "landscape", "content_filter": "high"},
                headers={"Authorization": f"Client-ID {UNSPLASH_ACCESS_KEY}"},
            )
            res.raise_for_status()
            results = res.json().get("results", [])
            if results:
                print(f"✅ Unsplash image found: {kw}")
                photo = results[0]
                return {
            ...
            print(f"⚠️ No Unsplash image for: {kw}")
        except Exception as e:
            print(f"⚠️ Unsplash failed for '{kw}': {e}")
    return None


def fetch_pexels_image(keyword: str, topic: str = "") -> dict:
    keywords_to_try = [keyword]
    if topic in TOPIC_FALLBACKS:
        keywords_to_try.append(TOPIC_FALLBACKS[topic])
    ...
    for kw in keywords_to_try:
        try:
            ...
            if photos:
                print(f"✅ Pexels image found: {kw}")
                photo = photos[0]
                return {
                    "url": photo["src"]["large"],
                    "alt": photo.get("alt") or kw,
                    "photographer": photo["photographer"],
                    "photographer_url": photo["photographer_url"],
                    "source": "Pexels",
                    "source_url": "https://www.pexels.com",
                }
            print(f"⚠️ No Pexels image for: {kw}")
        except Exception as e:
            print(f"⚠️ Pexels failed for '{kw}': {e}")
    return None


def fetch_image(keyword: str, topic: str = "") -> dict:
    """Try Unsplash first, fall back to Pexels."""
    image = fetch_unsplash_image(keyword, topic)
    if image:
        return image
    print("⚠️ Unsplash exhausted, trying Pexels...")
    return fetch_pexels_image(keyword, topic)
