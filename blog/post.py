...
- Format: Strictly follow this HTML structure:
    ...
    <p>Body paragraph...</p>
- Third to last line: [LABELS]3-5 specific keyword labels relevant to this exact article, comma-separated (e.g. "Mercedes-Benz, Electric Vehicle, Luxury Cars" or "Maldives, Six Senses, Overwater Villa, Luxury Travel")[/LABELS]
...
def generate_post(topic: str, recent_titles: list) -> dict:
    # Build recent titles string
    if recent_titles:
        recent_str = "\n".join(f"- {t}" for t in recent_titles)
    else:
        recent_str = "(none yet)"

    # Build full prompt with style guide + recent titles + topic prompt
    ...
    return {"title": title, "content": content, "meta": meta, "image_keyword": image_keyword, "labels": labels}
...
# ──────────────────────────────────────────
def post_to_blogger(title: str, content: str, labels: list, access_token: str):
    url = f"https://www.googleapis.com/blogger/v3/blogs/{BLOG_ID}/posts/"
    headers = {
...
def main():
    ...
    post_to_blogger(post["title"], content_with_image, post["labels"], access_token)
