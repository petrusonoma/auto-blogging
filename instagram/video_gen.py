...
def build_video(scenes: list[dict], image_paths: list[str], output_path: str, music_path: str = None) -> str:
    ...
    KB_EFFECTS = [
        # Pattern 1: Zoom in, center
        lambda f: (
            f"zoompan=z='min(zoom+0.0015,1.3)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d={f}:s={WIDTH}x{HEIGHT}:fps=25"
        ),
        # Pattern 2: Zoom out, pan right to left
        lambda f: (
            f"zoompan=z='if(lte(zoom,1.0),1.3,max(1.0,zoom-0.002))':x='iw/2-(iw/zoom/2)+20*on/{f}':y='ih/2-(ih/zoom/2)':d={f}:s={WIDTH}x{HEIGHT}:fps=25"
        ),
        # Pattern 3: Zoom in, pan bottom to top
        lambda f: (
            f"zoompan=z='min(zoom+0.001,1.25)':x='iw/2-(iw/zoom/2)':y='ih-(ih/zoom)-((ih-(ih/zoom))*on/{f})':d={f}:s={WIDTH}x{HEIGHT}:fps=25"
        ),
    ]
    ...
    for i, scene in enumerate(scenes):
        ...
        vf_parts = [
            # Scale to cover 9:16 frame without stretching (crop to fit)
            f"scale={WIDTH*2}:{HEIGHT*2}:force_original_aspect_ratio=increase,",
            f"crop={WIDTH*2}:{HEIGHT*2},",
            # Ken Burns zoompan (cycling pattern)
            f"{kb_effect},",
