...
Always write brand names in ALL CAPS (e.g. LORO PIANA, HEREND, CHRISTOFLE).
...
When mentioning luxury brands, always write them in ALL CAPS (e.g. LORO PIANA, HEREND).
Always respond in valid JSON only. No markdown, no preamble."""
...
CELEBRITY_SYSTEM = """You are a luxury lifestyle editor who connects Hollywood celebrities
with the quiet luxury brands they authentically wear and use.
You write with insider knowledge — specific, credible, never gossip-y.
Your tone is like a well-read friend who happens to know everyone's taste.
Never fabricate specific events. Focus on known style signatures and brand associations.
Always respond in valid JSON only. No markdown, no preamble."""

CELEBRITY_PROMPT = """Category: {category}

Write an Instagram Reel connecting a Hollywood celebrity to luxury {category} brands.
Choose a celebrity known for quiet, refined taste — not flashy. Examples: Cate Blanchett,
Sofia Coppola, Angelina Jolie, Meghan Markle, Gwyneth Paltrow, Jennifer Aniston.
Focus on their known style signature and the brands that match it.
Do NOT fabricate specific events or quotes. Stick to style associations.

Return this exact JSON structure:
{{
  "celebrity": "Celebrity Name",
  "scenes": [
    {{"duration": 3, "visual_hint": "specific {category} mood visual for image search", "overlay_text": "hook line, lowercase, max 5 words"}},
    {{"duration": 3, "visual_hint": "specific {category} mood visual for image search", "overlay_text": "one phrase, lowercase, max 5 words"}},
    {{"duration": 3, "visual_hint": "specific {category} mood visual for image search", "overlay_text": "one phrase, lowercase, max 5 words"}},
    {{"duration": 3, "visual_hint": "specific {category} mood visual for image search", "overlay_text": "one phrase, lowercase, max 5 words"}},
    {{"duration": 3, "visual_hint": "specific {category} mood visual for image search", "overlay_text": "closing phrase, lowercase, max 5 words"}}
  ],
  "caption": {{
    "hook": "One intriguing question or observation about this celebrity's style. Max 2 sentences. Can include 1-2 relevant emoji.",
    "story": "3-4 sentences connecting their style philosophy to specific luxury brands. Specific but never fabricated. No emoji.",
    "question": "One engagement question inviting the reader to share their own style icon. 1 sentence. Can include 1 emoji."
  }},
  "hashtags": [
    "STRATEGY: 2 large (5M+) including celebrity name tag, 6 medium (500K~5M), 6 niche (50K~500K). Include celebrity hashtag, brand hashtags, and style-specific niche tags. AVOID: #luxurylifestyle #homedecor #handcrafted.",
    "celebrity_name_tag",
    "quietluxury",
    "medium_tag1",
    "medium_tag2",
  ...
  "unsplash_queries": ["query1 celebrity style mood", "query2 brand aesthetic", "query3 fashion editorial", "query4 luxury mood", "query5 style detail"]
}}"""
...
def generate_script(category: str, post_format: str) -> dict:
    ...
    elif post_format == "celebrity":
        result = _call_claude(CELEBRITY_SYSTEM, CELEBRITY_PROMPT.format(category=category))
