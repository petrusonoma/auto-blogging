...
import sys
from schedule import get_post_config
...
def main():
    ...
    print("\n[3/5] Fetching images from Unsplash...")
    image_paths = fetch_images(script["unsplash_queries"])
    print(f"  ✓ {len(image_paths)} images fetched")

    # 4. Build video with ffmpeg
    print("\n[4/5] Rendering video...")
    output_video = "/tmp/reel_output.mp4"
    build_video(script["scenes"], image_paths, output_video)

    # 5. Publish to Instagram
    print("\n[5/5] Publishing to Instagram...")
