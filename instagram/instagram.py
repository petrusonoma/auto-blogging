...
def publish_reel(video_path: str, caption: str, hashtags: list[str]) -> str:
    ...
    """
    # Filter out STRATEGY placeholder and invalid tags, max 30 hashtags
    clean_tags = [
        t.lstrip('#') for t in hashtags
        if t
        and not t.upper().startswith("STRATEGY")
        and len(t) < 100
        and ' ' not in t.strip('#')
    ][:30]
    tag_string = " ".join(f"#{t}" for t in clean_tags)

    # Truncate caption to Instagram limit (2200 chars)
    max_caption = 2200 - len(tag_string) - 4
    truncated_caption = caption[:max_caption] if len(caption) > max_caption else caption
    full_caption = f"{truncated_caption}\n\n{tag_string}"
    ...
    if not container_resp.ok:
        print(f"  ✗ Instagram API error: {container_resp.status_code}")
        print(f"  Response: {container_resp.text}")
